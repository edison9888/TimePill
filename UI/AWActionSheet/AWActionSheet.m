...
-(id)initwithIconSheetDelegate:(id<AWActionSheetDelegate>)delegate ItemCount:(int)cout
{
    ...
    if (self) {
       ...
       self.scrollView = [[[UIScrollView alloc] initWithFrame:CGRectMake(0, 250, 320, 105)] autorelease];
        ...
        if (cout > 3) {
            /*被我改了*/
            self.pageControl = [[[UIPageControl alloc] initWithFrame:CGRectMake(0, 90, 0, 20)] autorelease];
...
- (void)reloadData
{
    ...
    if (count <= 3) {
        ...
        //[self setTitle:@"\n\n\n\n\n\n\n\n\n\n\n\n"];
        /*调整其Frame*/
       [scrollView setFrame:CGRectMake(0, -110, 320, 210)];
        rowCount = 2;
    }
    [scrollView setContentSize:CGSizeMake(320*(3/itemPerPage+1), scrollView.frame.size.height)];
