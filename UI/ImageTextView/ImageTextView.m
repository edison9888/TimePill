...
{    
    weiboEmotions=[[NSArray alloc] initWithObjects:@"[爱你]",@"[悲伤]",@"[闭嘴]",@"[馋嘴]",@"[吃惊]",@"[哈哈]",@"[害羞]",@"[汗]",@"[呵呵]",@"[黑线]",@"[花心]",@"[挤眼]",@"[可爱]",@"[可怜]",@"[酷]",@"[困]",@"[泪]",@"[生病]",@"[失望]",@"[偷笑]",@"[晕]",@"[挖鼻屎]",@"[阴险]",@"[囧]",@"[怒]",@"[good]",@"[给力]",@"[浮云]",@"[嘻嘻]",@"[鄙视]",@"[亲亲]",@"[太开心]",@"[懒得理你]",@"[右哼哼]",@"[左哼哼]",@"[嘘]",@"[衰]",@"[委屈]",@"[吐]",@"[打哈气]",@"[抱抱]",@"[疑问]",@"[拜拜]",@"[思考]",@"[睡觉]",@"[钱]",@"[哼]",@"[鼓掌]",@"[抓狂]",@"[怒骂]",@"[熊猫]",@"[奥特曼]",@"[猪头]",@"[蜡烛]",@"[蛋糕]",@"[din推撞]",nil];
...
-(CGPoint)drawSingleWord:(NSString *)text andX:(CGFloat)current_x andY:(CGFloat)current_y
{
    ...
    for (int i = 0; i < [text length]; i++)
    {
       ...
       // NSLog(@"draw%@",tem);
        CGSize size = [tem sizeWithFont:self.font constrainedToSize:CGSizeMake(self.frame.size.width, 2000)];
        // 14 * 18
        ...
        [tem drawInRect:CGRectMake(current_x, current_y, size.width, size.height) withFont:self.font];
