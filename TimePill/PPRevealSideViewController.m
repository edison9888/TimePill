...
- (void) popViewControllerAnimated:(BOOL)animated {
    HomePageViewController *home=(HomePageViewController *)self.rootViewController;
    [home setTableScrollEnable:YES];
    [self popViewControllerWithNewCenterController:_rootViewController
                                          animated:animated];
}
...
- (void) preloadViewController:(UIViewController*)controller forSide:(PPRevealSideDirection)direction withOffset:(CGFloat)offset {
    ...
    if (existingController != controller) {
        ...
        if (![controller isViewLoaded]) {
            ...
            //controller.view.hidden = YES;  //隐藏你妹啊！。。
        }
...
- (void) gestureRecognizerDidPan:(UIPanGestureRecognizer*)panGesture {
    ...
    if (_currentPanDirection == PPRevealSideDirectionNone) {
        ...
        /*已修改，防止斜向下滑动时，显示侧边栏*/
        if (panDiffX > 0 && panDiffX > OFFSET_TRIGGER_CHOSE_DIRECTION  && panDiffY<3.0  &&panDiffY>-3)
        {
           // NSLog(@"left");
            _currentPanDirection = PPRevealSideDirectionLeft;
            [home setTableScrollEnable:NO];
        }
        else
    ...
    //HomePageViewController *hh=(HomePageViewController *)self.rootViewController;
    //hh.view.userInteractionEnabled=NO;
    ...
    if (panGesture.state == UIGestureRecognizerStateEnded || panGesture.state == UIGestureRecognizerStateCancelled) {
        //NSLog(@"end");
        //HomePageViewController *hh=(HomePageViewController *)self.rootViewController;
        //hh.view.userInteractionEnabled=YES;
